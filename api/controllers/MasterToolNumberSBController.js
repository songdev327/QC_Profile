...
const app = express();
const MasterToolNumberSBModel = require("../models/MasterToolNumberSBModel");
const MasterToolNumberSpecSBModel = require("../models/MasterToolNumberSpecSBModel");

app.post("/masterToolNumber/masterToolNumberInsertSleeveSB", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberSBModel.create({
...
app.get("/masterToolNumber/masterToolNumberListSleeveSB", async (req, res) => {
  try {
    const results = await MasterToolNumberSBModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.post("/api/toolNumBerListNewSpecSleeveSB", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberSBModel.findAll({
      where: {
  ...
  const specs = await MasterToolNumberSpecSBModel.findAll({
    where: {
      Machine_Number: machineNumber.trim(),
      Partname_Model: partNameModel.trim(),
      process: processName.trim(),
...
app.put("/masterToolNumber/updateSleeveSB/:id", async (req, res) => {
  try {
    const { Machine_Number, Partname_Model, process, tool_no, password_input } = req.body;
    
    const result = await MasterToolNumberSBModel.update(
      {
...
app.delete("/machinesToolNumber/deleteSleeveSB/:id",  async (req, res) => {
  try {
    await MasterToolNumberSBModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/machinesToolNumber/listSearchToolSleeveSB", async (req, res) => {
  try {
    const results = await MasterToolNumberSBModel.findAll({
      order: [["id", "ASC"]],
    });
