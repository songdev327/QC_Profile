...
const app = express();
const MasterToolNumberSleeveModel = require("../models/MasterToolNumberSleeveModel");
const MasterToolNumberSpecSleeveModel = require("../models/MasterToolNumberSpecSleeveModel");

app.post("/masterToolNumber/masterToolNumberInsertSleeve", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberSleeveModel.create({
...
app.get("/masterToolNumber/masterToolNumberListSleeve", async (req, res) => {
  try {
    const results = await MasterToolNumberSleeveModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.post("/api/toolNumBerListNewSpecSleeve", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberSleeveModel.findAll({
      where: {
  ...
  const specs = await MasterToolNumberSpecSleeveModel.findAll({
    where: {
      Machine_Number: machineNumber.trim(),
      Partname_Model: partNameModel.trim(),
      process: processName.trim(),
    ...
    attributes: 
    ["tool_no", "section_check", "spec_tool_no", 
      "pass", "reject", "rev_control", "part_no", "spec_center"],
...
app.put("/masterToolNumber/updateSleeve/:id", async (req, res) => {
  try {
    const { Machine_Number, Partname_Model, process, tool_no, password_input } = req.body;
    
    const result = await MasterToolNumberSleeveModel.update(
      {
...
app.delete("/machinesToolNumber/deleteSleeve/:id",  async (req, res) => {
  try {
    await MasterToolNumberSleeveModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/machinesToolNumber/listSearchToolSleeve", async (req, res) => {
  try {
    const results = await MasterToolNumberSleeveModel.findAll({
      order: [["id", "ASC"]],
    });
