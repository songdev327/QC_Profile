...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecSleeveModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-ch", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL CH)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecSleeveModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecSleeveModel.sequelize,
        MasterToolNumberSpecSleeveModel.getTableName()
      );

      await MasterToolNumberSpecSleeveModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL CH สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool CH:", error);
