...
app.post("/masterNumberSleeve/masterNumberSpecToolInsertCS", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberSpecCSModel.create({
...
app.get("/masterNumberSleeve/masterNumberSpecNumberToolListCS", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecCSModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.put("/masterNumberSleeve/masterNumberSpecNumberToolupdateCS/:id", async (req, res) => {
  try {
    const { Machine_Number, Partname_Model, process, tool_no, section_check, 
      spec_tool_no, rev_control, part_no, password_input, 
      spec_center, date_control, div_control, sequence_number_spec, mesering_type } = req.body;

    const result = await MasterToolNumberSpecCSModel.update(
      {
        Machine_Number: Machine_Number,
        Partname_Model: Partname_Model,
        process: process,
...
app.delete("/masterNumberSleeve/masterNumberSpecNumberTooldeleteCS/:id",  async (req, res) => {
  try {
    await MasterToolNumberSpecCSModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/masterSpecToolSleeve/masterSpecToolSearchSleeveCS", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecCSModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.get("/masterSpecToolModel/masterSpecToolSearchModelSleeveCS", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecCSModel.findAll({
      order: [["id", "ASC"]],
    });
