...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecCSModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-cs", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL CS)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecCSModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecCSModel.sequelize,
        MasterToolNumberSpecCSModel.getTableName()
      );

      await MasterToolNumberSpecCSModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL CS สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool CS:", error);
