...
app.post("/masterNumber/masterNumberSpecToolInsert", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberSpecModel.create({
...
app.get("/masterNumber/masterNumberSpecNumberToolList", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.put("/masterNumber/masterNumberSpecNumberToolupdate/:id", async (req, res) => {
  try {
    const { Machine_Number, Partname_Model, process, tool_no, section_check, 
      spec_tool_no,rev_control, part_no, password_input, 
      spec_center, date_control, div_control, sequence_number_spec, mesering_type } = req.body;

    const result = await MasterToolNumberSpecModel.update(
      {
        Machine_Number: Machine_Number,
        Partname_Model: Partname_Model,
        process: process,
...
app.delete("/masterNumber/masterNumberSpecNumberTooldelete/:id",  async (req, res) => {
  try {
    await MasterToolNumberSpecModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.post("/api/toolNumBerList", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberSpecModel.findAll({
      where: {
...
app.post('/api/spec1TBS', async (req, res) => {
  const { tNumbers, machineNumber, partNameModel, productId } = req.body;
  if (!tNumbers || !machineNumber || !partNameModel || !productId) {
    return res.status(400).json({ message: 'Missing required parameters' });
  }
  ...
  // ปรับให้เป็น lower ทั้งหมดเพื่อเทียบแบบไม่สนตัวพิมพ์และตัดช่องว่าง
  const tNumsLower = tNumbers.map(norm);
  const machineLower = norm(machineNumber);
  const modelLower = norm(partNameModel);

  try {
    const specData = await ProductInputSpecModel.findAll({
      where: {
        [Op.and]: [
          // TRIM(LOWER(tool_no)) IN (:tNumsLower)
          where(fn('LOWER', fn('TRIM', col('tool_no'))), { [Op.in]: tNumsLower }),
          // TRIM(LOWER(Machine_Number)) = :machineLower
          where(fn('LOWER', fn('TRIM', col('Machine_Number'))), machineLower),
          // TRIM(LOWER(Partname_Model)) = :modelLower
          where(fn('LOWER', fn('TRIM', col('Partname_Model'))), modelLower),
          // productId ต้องตรง
          { productId }
        ]
      },
    ...
    // console.log('found tool_no:', specData.map(s => s.tool_no));
    // DEBUG (ถ้าต้องการตรวจค่า)
    // console.log({ tNumbers, tNumsLower, machineNumber, partNameModel, productId, found: specData.length });
...
app.get("/masterSpecTool/masterSpecToolSearch", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecModel.findAll({
      order: [["id", "ASC"]],
    });
