...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecSBModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-sb", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL SB)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecSBModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecSBModel.sequelize,
        MasterToolNumberSpecSBModel.getTableName()
      );

      await MasterToolNumberSpecSBModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL SB สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool SB:", error);
