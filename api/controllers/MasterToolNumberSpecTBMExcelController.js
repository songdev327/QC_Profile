...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecTBMModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-tbm", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL TBM)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecTBMModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecTBMModel.sequelize,
        MasterToolNumberSpecTBMModel.getTableName()
      );

      await MasterToolNumberSpecTBMModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL TBM สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool TBM:", error);
