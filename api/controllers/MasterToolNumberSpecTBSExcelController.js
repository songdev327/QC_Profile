...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-tbs", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL TBS)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecModel.sequelize,
        MasterToolNumberSpecModel.getTableName()
      );

      await MasterToolNumberSpecModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL TBS สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool TBS:", error);
