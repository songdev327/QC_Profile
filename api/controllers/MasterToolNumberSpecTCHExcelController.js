...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecTCHModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-tch", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL TCH)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecTCHModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecTCHModel.sequelize,
        MasterToolNumberSpecTCHModel.getTableName()
      );

      await MasterToolNumberSpecTCHModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL TCH สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool TCH:", error);
