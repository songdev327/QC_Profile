...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecTNModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-tn", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL TN)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecTNModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecTNModel.sequelize,
        MasterToolNumberSpecTNModel.getTableName()
      );

      await MasterToolNumberSpecTNModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL TN สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool TN:", error);
