...
app.post("/masterNumber/masterNumberSpecToolInsertTTC", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberSpecTTCModel.create({
...
app.get("/masterNumber/masterNumberSpecNumberToolListTTC", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecTTCModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.put("/masterNumber/masterNumberSpecNumberToolupdateTTC/:id", async (req, res) => {
  try {
    const { Machine_Number, Partname_Model, process, tool_no, 
      section_check, spec_tool_no,rev_control, part_no, password_input, 
      spec_center, date_control, div_control, sequence_number_spec, mesering_type  } = req.body;

    const result = await MasterToolNumberSpecTTCModel.update(
      {
        Machine_Number: Machine_Number,
        Partname_Model: Partname_Model,
        process: process,
...
app.delete("/masterNumber/masterNumberSpecNumberTooldeleteTTC/:id",  async (req, res) => {
  try {
    await MasterToolNumberSpecTTCModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.post("/api/toolNumBerListTTC", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberSpecTTCModel.findAll({
      where: {
...
app.get("/masterSpecTool/masterSpecToolSearchTTC", async (req, res) => {
  try {
    const results = await MasterToolNumberSpecTTCModel.findAll({
      order: [["id", "ASC"]],
    });
