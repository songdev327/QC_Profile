...
function getDbHeaders() {
  return Object.keys(MasterToolNumberSpecTTCModel.rawAttributes).filter(
...
app.post("/upload-master-tool-spec-ttc", (req, res) => {
    ...
    try {
      ...
      if (ext === ".xlsx") {
        ...
        if (missingInExcel.length || extraInExcel.length) {
          return res.status(400).json({
            message: "⚠️ หัวตารางในไฟล์ Excel ไม่ตรงกับหัวตารางในฐานข้อมูล (MASTER TOOL TTC)",
            details: {
              expected: dbHeaders,
              found: excelHeaders,
              missingInExcel,
      ...
      await MasterToolNumberSpecTTCModel.destroy({ where: {} });

      await restartIdSequence(
        MasterToolNumberSpecTTCModel.sequelize,
        MasterToolNumberSpecTTCModel.getTableName()
      );

      await MasterToolNumberSpecTTCModel.bulkCreate(rows);

      return res.status(200).json({
        message: `✅ นำเข้า MASTER TOOL TTC สำเร็จ (${rows.length} แถว)`,
      });
    } catch (error) {
      console.error("❌ Error importing MasterTool TTC:", error);
