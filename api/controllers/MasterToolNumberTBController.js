...
const MasterToolNumberTBModel = require("../models/MasterToolNumberTBModel");
const MasterToolNumberSpecTBModel = require("../models/MasterToolNumberSpecTBModel");

app.post("/masterToolNumber/masterToolNumberInsertTB", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberTBModel.create({
...
app.get("/masterToolNumber/masterToolNumberListTB", async (req, res) => {
  try {
    const results = await MasterToolNumberTBModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.post("/api/toolNumBerListTB", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberTBModel.findAll({
      where: {
...
app.post("/api/toolNumBerListNewSpecTB", async (req, res) => {
  ...
  const specs = await MasterToolNumberSpecTBModel.findAll({
    where: {
      Machine_Number: machineNumber.trim(),
      Partname_Model: partNameModel.trim(),
      process: processName.trim(),
...
app.put("/masterToolNumber/updateTB/:id", async (req, res) => {
  try {
    ...
    const result = await MasterToolNumberTBModel.update(
      {
...
app.delete("/machinesToolNumber/deleteTB/:id",  async (req, res) => {
  try {
    await MasterToolNumberTBModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/machinesToolNumber/listSearchToolTB", async (req, res) => {
  try {
    const results = await MasterToolNumberTBModel.findAll({
      order: [["id", "ASC"]],
    });
