...
const MasterToolNumberTBMModel = require("../models/MasterToolNumberTBMModel");
const MasterToolNumberSpecTBMModel = require("../models/MasterToolNumberSpecTBMModel");

app.post("/masterToolNumber/masterToolNumberInsertTBM", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberTBMModel.create({
...
app.get("/masterToolNumber/masterToolNumberListTBM", async (req, res) => {
  try {
    const results = await MasterToolNumberTBMModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.post("/api/toolNumBerListTBM", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberTBMModel.findAll({
      where: {
...
app.post("/api/toolNumBerListNewSpecTBM", async (req, res) => {
  ...
  const specs = await MasterToolNumberSpecTBMModel.findAll({
    where: {
      Machine_Number: machineNumber.trim(),
      Partname_Model: partNameModel.trim(),
      process: processName.trim(),
...
app.put("/masterToolNumber/updateTBM/:id", async (req, res) => {
  try {
    ...
    const result = await MasterToolNumberTBMModel.update(
      {
...
app.delete("/machinesToolNumber/deleteTBM/:id",  async (req, res) => {
  try {
    await MasterToolNumberTBMModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/machinesToolNumber/listSearchToolTBM", async (req, res) => {
  try {
    const results = await MasterToolNumberTBMModel.findAll({
      order: [["id", "ASC"]],
    });
