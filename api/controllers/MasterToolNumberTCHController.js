...
const MasterToolNumberTCHModel = require("../models/MasterToolNumberTCHModel");
const MasterToolNumberSpecTCHModel = require("../models/MasterToolNumberSpecTCHModel");

app.post("/masterToolNumber/masterToolNumberInsertTCH", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberTCHModel.create({
...
app.get("/masterToolNumber/masterToolNumberListTCH", async (req, res) => {
  try {
    const results = await MasterToolNumberTCHModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.post("/api/toolNumBerListTCH", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberTCHModel.findAll({
      where: {
...
app.post("/api/toolNumBerListNewSpecTCH", async (req, res) => {
  ...
  const specs = await MasterToolNumberSpecTCHModel.findAll({
    where: {
      Machine_Number: machineNumber.trim(),
      Partname_Model: partNameModel.trim(),
      process: processName.trim(),
...
app.put("/masterToolNumber/updateTCH/:id", async (req, res) => {
  try {
    ...
    const result = await MasterToolNumberTCHModel.update(
      {
...
app.delete("/machinesToolNumber/deleteTCH/:id",  async (req, res) => {
  try {
    await MasterToolNumberTCHModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/machinesToolNumber/listSearchToolTCH", async (req, res) => {
  try {
    const results = await MasterToolNumberTCHModel.findAll({
      order: [["id", "ASC"]],
    });
