...
const app = express();
const MasterToolNumberTNModel = require("../models/MasterToolNumberTNModel");
const MasterToolNumberSpecTNModel = require("../models/MasterToolNumberSpecTNModel");

app.post("/masterToolNumber/masterToolNumberInsertSleeveTN", async (req, res) => {
  ...
  try {
    ...
    let result = await MasterToolNumberTNModel.create({
...
app.get("/masterToolNumber/masterToolNumberListSleeveTN", async (req, res) => {
  try {
    const results = await MasterToolNumberTNModel.findAll({
      order: [["id", "ASC"]],
    });
...
app.post("/api/toolNumBerListNewSpecSleeveTN", async (req, res) => {
  ...
  try {
    const toolNumbers = await MasterToolNumberTNModel.findAll({
      where: {
  ...
  const specs = await MasterToolNumberSpecTNModel.findAll({
    where: {
      Machine_Number: machineNumber.trim(),
      Partname_Model: partNameModel.trim(),
      process: processName.trim(),
...
app.put("/masterToolNumber/updateSleeveTN/:id", async (req, res) => {
  try {
    const { Machine_Number, Partname_Model, process, tool_no, password_input } = req.body;
    
    const result = await MasterToolNumberTNModel.update(
      {
...
app.delete("/machinesToolNumber/deleteSleeveTN/:id",  async (req, res) => {
  try {
    await MasterToolNumberTNModel.destroy({
      where: {
        id: req.params.id,
      },
    });
...
app.get("/machinesToolNumber/listSearchToolSleeveTN", async (req, res) => {
  try {
    const results = await MasterToolNumberTNModel.findAll({
      order: [["id", "ASC"]],
    });
