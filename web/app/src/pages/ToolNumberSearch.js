...
function ToolNumberSearch(props) {
  ...
  const fetchDataSearch = async (machine) => {
    try {
      ...
      const response = await axios.post(
        config.api_path + "/product/ProductMCQC_CH",
  ...
  const handlePageClick = async (data) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/product/listNewProductionSleeve?offset=${offset}&limit=${limit}`,
        config.headers()
      );
  ...
  const getMachines = async () => {
    try {
      const response = await axios.get(config.api_path + "/machinesCH/listOnlyTool");
  ...
  const getMachinesSearch = async () => {
    try {
      const response = await axios.get(config.api_path + "/getDataMCCHSearch");
  ...
  const handleMachineSelect = async (selectedOption) => {
    // เคลียร์ค่าเก่าของ model และ process ก่อน
    setModel("");
    setProcess("");
    setIp_address("");
    setSystem_no("");
    ...
    setSelectedMachine(selectedOption.value); // เก็บ machine number ที่เลือกใน state

    try {
      const response = await axios.get(
        `${config.api_path}/machines/${selectedOption.value}/detailsCH`
      );
      ...
      if (response.data.message === "success") {
        ...
        setSystem_no(response.data.system_no);
        setAddress_type(response.data.address_type);
        setAddress_no(response.data.address_no);
        ...
        setProduct({
          ...
          system_no: response.data.system_no,
          address_type: response.data.address_type,
          address_no: response.data.address_no,
        });
  ...
  const handleSaveAfterSet = async (e) => {
    e.preventDefault();
    if (!af1Checked) {
      Swal.fire({
        title: "แจ้งเตือน",
    ...
    if (!nameafterset || nameafterset.trim() === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณากรอกชื่อในการ AF1",
        icon: "warning",
      });
      return;
    }
    try {
      ...
      let payload = {
        ...
        dateafterset: dateafterset,
        timeafterset: timeafterset,
      };
      await axios.post(url, payload, config.headers()).then((res) => {
        if (res.data.message === "success") {
          Swal.fire({
            title: "บันทึกข้อมูล",
            text: "บันทึก After Set แล้ว",
            icon: "success",
            showConfirmButton: true,
            showCancelButton: true,
          });
  ...
  const handleSaveAfterSet2 = async (e) => {
    e.preventDefault();
    if (!af2Checked) {
      Swal.fire({
        title: "แจ้งเตือน",
    ...
    if (!nameafterset2 || nameafterset2.trim() === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณากรอกชื่อในการ AF2",
        icon: "warning",
      });
      return;
    }
    try {
      ...
      let payload = {
        ...product,
        nameafterset2: nameafterset2,
        dateafterset2: dateafterset2,
        timeafterset2: timeafterset2,
      };
      await axios.post(url, payload, config.headers()).then((res) => {
        if (res.data.message === "success") {
          Swal.fire({
  ...
  const handleSaveAfterSet3 = async (e) => {
    e.preventDefault();
    if (!af3Checked) {
      Swal.fire({
        title: "แจ้งเตือน",
    ...
    if (!nameafterset3 || nameafterset3.trim() === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณากรอกชื่อในการ AF3",
        icon: "warning",
      });
      return;
    }
    try {
      ...
      let payload = {
        ...product,
        nameafterset3: nameafterset3,
        dateafterset3: dateafterset2,
        timeafterset3: timeafterset2,
      };
      await axios.post(url, payload, config.headers()).then((res) => {
        if (res.data.message === "success") {
          Swal.fire({
  ...
  const handleSaveAfterSet4 = async (e) => {
    e.preventDefault();
    if (!af4Checked) {
      Swal.fire({
        title: "แจ้งเตือน",
    ...
    if (!nameafterset4 || nameafterset4.trim() === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณากรอกชื่อในการ AF4",
        icon: "warning",
      });
      return;
    }
    try {
      ...
      let payload = {
        ...product,
        nameafterset4: nameafterset4,
        dateafterset4: dateafterset2,
        timeafterset4: timeafterset2,
      };
      await axios.post(url, payload, config.headers()).then((res) => {
        if (res.data.message === "success") {
          Swal.fire({
  ...
  const handleSaveAfterSet5 = async (e) => {
    e.preventDefault();
    if (!af5Checked) {
      Swal.fire({
        title: "แจ้งเตือน",
    ...
    if (!nameafterset5 || nameafterset5.trim() === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณากรอกชื่อในการ AF5",
        icon: "warning",
      });
      return;
    }
    try {
      ...
      let payload = {
        ...product,
        nameafterset5: nameafterset5,
        dateafterset5: dateafterset2,
        timeafterset5: timeafterset2,
      };
      await axios.post(url, payload, config.headers()).then((res) => {
        if (res.data.message === "success") {
          Swal.fire({
  ...
  const handleSaveQCBYoff = async (e) => {
    ...
    if (document.getElementById("name_qc_by_off").value === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณากรอกชื่อ Confime QC BY OFF",
        icon: "warning",
      });
      return;
    }
    if (product.machine && product.machine.startsWith("TN")) {
      Swal.fire({
        title: "ไม่สามารถเพิ่มข้อมูลได้",
        text: "กรุณาเลือก SELECT M/C TYPE : TN",
        icon: "error",
      });
      return;
    }
  ...
  const handleSaveQCProjector = async (e) => {
    e.preventDefault();
    if (document.getElementById("projectorValue").value === "") {
      Swal.fire({
        title: "แจ้งเตือน",
        text: "กรุณาเลือก Projector Status",
        icon: "warning",
      });
      return;
    }
    try {
      ...
      if (product.id !== undefined) {
        url = config.api_path + "/product/updateDetailProjector";
      }
      let payload = {
        ...product,
        projector_status: projectorStatus,
        name_qc_projector_check: name_qc_projector_check,
        date_qc_projector_check: date_qc_projector_check,
        time_qc_projector_check: time_qc_projector_check,
      };

      console.log("Payload being sent to backend:", payload);
      if (projectorStatus === "OK") {
        payload.barcode = "Pass";
      }
      await axios.post(url, payload, config.headers()).then((res) => {
        if (res.data.message === "success") {
          Swal.fire({
            title: "บันทึกข้อมูล",
            text: "บันทึก Projector Status แล้ว",
            icon: "success",
            timer: 3000,
          });
          if (projectorStatus === "OK") {
  ...
  const handleStatusChange = (e) => {
    setProjectorStatus(e.target.value);
  };

  const openModalProjector = () => {
    const modalElement = document.getElementById("modalProjector");
    const modalInstance = new window.bootstrap.Modal(modalElement);
    modalInstance.show();
  };

  const closeModalProjector = () => {
    const modalElement = document.getElementById("modalProjector");
    if (modalElement) {
    ...
    try {
      ...
      if (qcData.length > 0) {
        const updatedSpecData = qcData.map((item) => ({
          ...
          qc_line_input: item.qc_line_input,
          end_time_qc_line: item.end_time_qc_line,
          pass: item.pass,
          reject: item.reject,
        }));

        setSpecData(updatedSpecData);
        setInputValues(updatedSpecData.map((item) => item.qc_line_input || ""));
        setPassChecked(updatedSpecData.map((item) => item.pass === "Pass"));
        setRejectChecked(
          updatedSpecData.map((item) => item.reject === "Reject")
        );
      }
    } catch (error) {
  ...
  const handleOkClickToolNumberList = async () => {
    try {
      ...
      const response = await axios.post(
        `${config.api_path}/api/toolNumBerListNewSpecSleeve`,
        {
          machineNumber: machineNumber,
  ...
  return (
    <>
          ...
          {filteredProducts.length > 0
            ? filteredProducts.map((item) => (
              <tr>
                ...
                <td>{item.process}</td>
                <td>
                  {Array.from({ length: 42 }, (_, i) => item[`t${i + 1}`]).some(val => val)
                    ? Array.from({ length: 42 }, (_, i) => item[`t${i + 1}`]).join("-")
                    : ""}
                </td>
                      ...
                      <>
                        <div className="d-flex justify-content-between align-items-center mt-3 mb-2">
                          <div className="form-check tool-item mt-3">
                            ...
                            />
                            <label
                              className="form-check-label fw-bold ml-4"
                              htmlFor="select-all"
        ...
        <div className="row">
          ...
          {/* {!hasTNMachine && (
            <div className="col-3 mt-2">
              ...
              <input
                onChange={(e) =>
                  setName_qc_by_off(e.target.value.toUpperCase())
                }
                className="form-control text-primary"
                name="name_qc_by_off"
                id="name_qc_by_off"
      ...
      {/*------ Start Modal Projector ---------------------------------------------------------*/}
      <ModalProjector id="modalProjector" title="" modalSize="modal-lg">
        <h3 className="h3 projector-check">
          <b className="ml-3">PROJECTOR CHECK</b>
        </h3>
        <div className="row">
          <div className="col-4 mt-2 ml-2">
            ...
            <input
              value={product.name_qc_by_off || name_qc_projector_check} // แสดงค่าจาก product.name_qc_by_off หรือจาก state
              className="form-control text-dark fw-bold text-center"
              disabled // ป้องกันไม่ให้ผู้ใช้แก้ไข
            />
          </div>
          <div className="col-2 mt-2">
            <div className="text-bold text-center" id="qc-inprocess-confirm">
            ...
            <input
              value={date_qc_projector_check}
              onChange={(e) => setDate_qc_projector_check(e.target.value)}
              className="form-control text-dark fw-bold text-center"
              disabled
            />
          </div>
          <div className="col-2 mt-2">
            ...
            <input
              value={time_qc_projector_check}
              onChange={(e) => setTime_qc_projector_check(e.target.value)}
              className="form-control text-dark fw-bold text-center"
              disabled
            />
          </div>
        ...
        <div className="row">
          {/* แสดง input เฉพาะเมื่อเลือก NG */}
          {projectorStatus === "NG" && (
            <>
              <div className="col-10 mt-4 ml-2">
                <button
                  className="btn btn-success mb-3"
                  onClick={handleSaveQCProjector}
                >
                  Save Status Projector Ng
                </button>
              </div>
              <hr></hr>
              <div className="col-2 mt-1">
                <div className="text-bold mt-3 ml-3" id="">
                  Mesering
                </div>
                <input
                  id="projectorValue"
              ...
              <div className="col-2 mt-1">
                <div className="text-bold mt-3 ml-1" id="">
                  Status
                </div>
                <input
                  id="projectorValue"
              ...
              <div className="col-6 mt-3" id="box-1">
                <div className="text-bold pl-1 ">Image status</div>
                <input
      ...
      <ModalQCInprocessShaft id="modalContourOverTarget" modalSize="modal-lg">
        ...
        <div className="col-10">
          ...
          {/* {!nameQcLine && !specData.some(spec => spec.tool_no && spec.spec) && !product.time_qc_by_off && ( // ถ้า nameQcLine ไม่มีค่า (null หรือว่าง) แสดงปุ่ม Check
            <button className="btn btn-primary mb-3 blinking-button" onClick={handleOkClickContourOverTarget}>
              Check
            </button>
          )} */}
        </div>
        ...
        {/* {hasTNMachine && product.area_qc_check === "QC Line" && (
          (product.qcline_status === "OK" || product.qcline_status === "NG") &&
          <button className="btn btn-success mb-3 ml-1"
            onClick={handleSaveSpecQCLineOnly_TN_input_data}
          >
            Update Name QC Line TN?
          </button>
        )} */}
        <hr />
        <div className="d-flex justify-content-end">
...
export default ToolNumberSearch;
