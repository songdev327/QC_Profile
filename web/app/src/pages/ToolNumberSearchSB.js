...
function ToolNumberSearchSB(props) {
  ...
  const handlePageClick = async (data) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/product/listNewProductionSleeveSB?offset=${offset}&limit=${limit}`,
        config.headers()
      );
  ...
  const getMachines = async () => {
    try {
      const response = await axios.get(config.api_path + "/machinesSB/listOnlyTool");
  ...
  const handleOkClickToolNumberList = async () => {
    try {
      ...
      const response = await axios.post(
        `${config.api_path}/api/toolNumBerListNewSpecSleeveSB`,
        {
...
export default ToolNumberSearchSB;
