...
// helper: ดึง T ทั้งหมดจาก product และ map t43 -> "Rough"
// const buildTNumbersForModal = (productObj) => {
//   return Object.entries(productObj)
//     .filter(([k, v]) => /^t\d+$/.test(k) && v)
//     .map(([k, v]) => (k.toLowerCase() === "t43" ? "Rough" : v));
// };

function ToolNumberSearchShaft(props) {
  ...
  const [end_time_qc_lineAuto, setEnd_time_qc_lineAuto] = useState(""); // แก้ไขให้ Shaft 29-08-25
  ...
  const [checkboxValues, setCheckboxValues] = useState({
    oil: false,
    air: false,
    sensor_check_tap: false,
    sensor_check_1: false,
    sensor_check_2: false,
  });
  ...
  const handleSelectAllChange = () => {
    ...
    setCheckboxValues({
      oil: !selectAll,
      air: !selectAll,
      sensor_check_tap: !selectAll,
      sensor_check_1: !selectAll,
      sensor_check_2: !selectAll,
    });
    setProduct({
      ...product,
      oil: !selectAll ? "OK" : "",
      air: !selectAll ? "OK" : "",
      sensor_check_tap: !selectAll ? "OK" : "",
      sensor_check_1: !selectAll ? "OK" : "",
      sensor_check_2: !selectAll ? "OK" : "",
    });
  };
  ...
  useEffect(() => {
    ...
    const nowQCLine = new Date();
    const formattedEndTimeQCLine = nowQCLine.toTimeString().slice(0, 8);
    setEnd_time_qc_line(formattedEndTimeQCLine);
  }, []);
  ...
  const fetchDataSearch = async (machine) => {
    try {
      ...
      const response = await axios.post(
        config.api_path + "/product/ProductMCQC_TBS",
  ...
  const handlePageClick = async (data) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/product/listNewMachineShaft?offset=${offset}&limit=${limit}`,
        config.headers()
      );
  ...
  const getMachines = async () => {
    try {
      const response = await axios.get(config.api_path + "/machinesTBS/listShaft");
  ...
  const getMachinesSearch = async () => {
    try {
      const response = await axios.get(config.api_path + "/getDataMCTBSSearch");
  ...
  const handleMachineSelect = async (selectedOption) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/machinesShaft/${selectedOption.value}/details`
      );
      if (response.data.message === "success") {
  ...
  const handleSaveQCBYoff = async (e) => {
    e.preventDefault();
    try {
      let url = config.api_path + "/product/insert";
      if (product.id !== undefined) {
  ...
  useEffect(() => { }, [product.time_qc_by_off]);
  ...
  const handleMCTypeChange = (value) => {
    ...
    if (value === "TBS") {
      ...
      navigate("/toolNumberSearchShaftTBM");
      window.location.reload();
    } else if (value === "TTC") {
      navigate("/toolNumberSearchShaftTTC");
      window.location.reload();
    } else if (value === "TCH") {
      navigate("/toolNumberSearchShaftTCH");
      window.location.reload();
    } else if (value === "TB") {
      navigate("/toolNumberSearchShaftTB");
      window.location.reload();
    } else {
  ...
  useEffect(() => {
    const filtered = products.filter((item) => {
      if (selectedMCType === "TB") {
        return item.machine.startsWith("TB-");
      }
  ...
  const handleOkClickContourOverTarget = async () => {
    try {
      const normKey = (s) => String(s || "").trim().toLowerCase();

      // 1) จาก selectedOverTarget (ถ้ามีกรอก)
      let tNumbers = Object.keys(selectedOverTarget || {})
        .filter((k) => selectedOverTarget[k] && selectedOverTarget[k] !== "")
        .map((k) => {
          const keyLower = normKey(k);
          if (keyLower === "t43") return "Rough";         // ✅ map t43 -> Rough
          const num = keyLower.replace(/^t/i, "");
          return "T" + num.padStart(2, "0");              // T01..T42
        });

      // 2) Fallback: ดึงจาก product.t* ถ้ายังไม่มี
      if (!tNumbers.length) {
        tNumbers = Object.entries(product || {})
          .filter(([k, v]) => /^t\d+$/i.test(k) && v)
          .map(([k]) => {
            const keyLower = normKey(k);
            if (keyLower === "t43") return "Rough";       // ✅ map t43 -> Rough
            const num = keyLower.replace(/^t/i, "");
      ...
      // 3) ถ้า product มี t43 ให้ยัด Rough เข้าไปชัวร์ ๆ
      if (normKey(product?.t43)) {
        if (!tNumbers.map((t) => t.toLowerCase()).includes("rough")) {
          tNumbers.push("Rough");
        }
      }

      // 4) หรือถ้า list เครื่องมือมีตัวชื่อ Rough ก็ยัดด้วย
      if (Array.isArray(toolNumbers) && toolNumbers.some(t => normKey(t.tool_no) === "rough")) {
        if (!tNumbers.map((t) => t.toLowerCase()).includes("rough")) {
          tNumbers.push("Rough");
        }
      }
      ...
      // 5) กันค่าซ้ำ
      tNumbers = Array.from(new Set(tNumbers));

      // (debug) ดูว่า Rough ติดมาหรือยัง
      // console.log("tNumbers to /api/spec1 =>", tNumbers);

      const response = await axios.post(
        `${config.api_path}/api/spec1TBS`,
        {
          tNumbers,
  ...
  const handleUpdateClick = async () => {
    try {
      setSpecData([]);
      const response = await axios.get(
        `${config.api_path}/api/qc-line-input-updateNew/${product.id}`
      );
      const qcData = response.data;
      if (qcData.length > 0) {
        const updatedSpecData = qcData.map((item) => ({
          id: item.id,
          tool_no: item.tool_no,
          section_check: item.section_check,
          spec_tool_no: item.spec_tool_no,
          spec_tool_no_input: item.spec_tool_no_input,
          rev_control: item.rev_control,
          pass: item.pass,
          reject: item.reject,
        }));
        setSpecData(updatedSpecData);
        setInputValues(
          updatedSpecData.map((item) => item.spec_tool_no_input || "")
        );
        setPassChecked(updatedSpecData.map((item) => item.pass === "Yes"));
        setRejectChecked(updatedSpecData.map((item) => item.reject === "Yes"));
      }
    } catch (error) {
      console.error("Error fetching QC Line Input data:", error);
    }
  };
  ...
  const openModalContourOverTarget = async () => {
    try {
      ...
      const response = await axios.get(
        `${config.api_path}/api/qc-line-input-latestNew/${product.id}`
      );
      const latestQcData = response.data;
      setNameQcLine(latestQcData?.name_qc_line || "");
      if (!latestQcData?.time_qc_by_off) {
        handleOkClickContourOverTarget();
      } else {
        handleUpdateClick();
      }
      modalInstance.show();
  ...
  const handleSaveQCBYoffTNNew = async (e) => {
    e.preventDefault();
    try {
      let url = config.api_path + "/product/insert";
      if (product.id !== undefined) {
  ...
  const handleOkClickToolNumberList = async () => {
    try {
      ...
      const response = await axios.post(
        `${config.api_path}/api/toolNumBerListNewSpec`,
        {
      ...
      const fetchedToolNumbers = response.data.map((item) => {
        const toolNo = item.tool_no || "";
        const isRough = /rough/i.test(toolNo); // ตรวจว่ามีคำว่า Rough (ไม่แคสตัวพิมพ์)
        return {
          tool_no: toolNo,
          selected: isRough ? true : false,   // ✅ Rough ถูกเลือกอัตโนมัติ
          specs: item.specs || [],
          statusOptions: { complete: false, burr: false, broken: false, cf: false },
        };
      });
  ...
  const handleSaveNewAuto = async (e) => {
    ...
    const isRoughTool = (toolNo) => /rough/i.test(toolNo || ""); // true ถ้าเป็น Rough
    ...
    try {
      // สร้าง mapping t1..t42 ตาม tool_no ที่เป็น Txx
      const selectedTools = {};
      ...
      toolNumbers.forEach((tool) => {
        ...
        // map เฉพาะที่เป็นรูปแบบ t1..t42
        if (isRoughTool(tool.tool_no)) {
          // ✅ ถ้าเป็น Rough → บันทึกเข้า t43
          selectedTools["t43"] = tool.tool_no;
        } else if (toolNo.startsWith("t")) {
          const toolKey = parseInt(toolNo.slice(1), 10);
          if (!Number.isNaN(toolKey) && toolKey >= 1 && toolKey <= 42) {
            selectedTools[`t${toolKey}`] = tool.tool_no;
          }
        }

        // เก็บสถานะ (ถ้าไม่ใช่ Rough ถึงจะมีตัวเลือก)
        if (!isRoughTool(tool.tool_no)) {
          if (tool.statusOptions.complete) toolChangeCases.push(`${tool.tool_no}, Complete`);
          if (tool.statusOptions.burr) toolChangeCases.push(`${tool.tool_no}, Burr`);
          if (tool.statusOptions.broken) toolChangeCases.push(`${tool.tool_no}, Broken`);
          if (tool.statusOptions.cf) toolChangeCases.push(`${tool.tool_no}, C/F`);
        }
      });

      // ✅ บังคับให้เลือกสถานะ "เฉพาะเครื่องมือที่ไม่ใช่ Rough"
      const needStatusButMissing = toolNumbers.some((tool) => {
        if (!tool.selected) return false;
        if (isRoughTool(tool.tool_no)) return false; // Rough ไม่ต้องเลือกสถานะ
        // ถ้าไม่ใช่ Rough ต้องมีเลือกอย่างน้อย 1 สถานะ
        return !Object.values(tool.statusOptions || {}).some(Boolean);
      });

      if (needStatusButMissing) {
        Swal.fire({
          title: "กรุณาเลือกสถานะ",
          text: "กรุณาเลือกสถานะสำหรับ Tool Number ที่เลือก (ยกเว้น Rough)",
          icon: "warning",
        });
        return;
      }

      // ✅ ใหม่: Production Check ต้องเลือก Pass/Reject อย่างน้อย 1
      const prodCheckMissing = [];
      toolNumbers.forEach((tool) => {
        if (!tool.selected) return;
        (tool.specs || []).forEach((spec) => {
          const isProductionCheck = String(spec.section_check).trim() === "Production Check";
          const noPass = spec.pass !== "Yes";
          const noReject = spec.reject !== "Yes";
          if (isProductionCheck && noPass && noReject) {
            prodCheckMissing.push(`${tool.tool_no} : ${spec.spec_tool_no || "-"}`);
          }
        });
      });
      if (prodCheckMissing.length > 0) {
        Swal.fire({
          icon: "warning",
          title: "กรุณาเลือก Pass หรือ Reject",
          html: `
          พบสเปคที่ยังไม่ได้เลือกผล (Production Check):<br/>
          <div style="text-align:left;margin-top:8px">
            ${prodCheckMissing.map((t) => `• ${t}`).join("<br/>")}
          </div>
        `,
        });
        return;
      ...
      const payload = {
        ...product,
        ...selectedTools,
        origin_barcode: product.barcode,
      ...
      if (productResponse.data.message === "success") {
        ...
        // เตรียมข้อมูลสำหรับ ProductInputSpecModel
        ...
        toolNumbers.forEach((tool, toolIndex) => {
          ...
          tool.specs.forEach((spec, specIndex) => {
            const toolStatus = [];
            // ถ้าเป็น Rough จะไม่ถูกแสดง/เลือกอยู่แล้ว (ว่างไว้)
            if (!isRoughTool(tool.tool_no)) {
              ...
              if (tool.statusOptions.cf) toolStatus.push("C/F");
            }
        ...
        await axios
          ...
          .catch((error) => {
            console.error("Error inserting data to ProductInputSpecModel:", error);
          });
      }
    } catch (e) {
      Swal.fire({
  ...
  const handleSaveSpecQCLine = async (e) => {
    e.preventDefault();
    if (!product.id) {
      Swal.fire({
        title: "Error",
    ...
    // ✅ ต้องกรอกค่า input สำหรับ Production Check / QC Line Check
    const MUST_INPUT = new Set(["Production Check", "QC Line Check"]);
    const missingInput = [];

    (specData || []).forEach((spec, idx) => {
      const sec = String(spec.section_check || "").trim();
      const need = MUST_INPUT.has(sec);

      // ค่าที่ผู้ใช้กรอก (ถ้าไม่กรอกให้ดูค่าที่เคยมีในฐานข้อมูล)
      const val =
        (inputValues?.[idx] ?? "").toString().trim() ||
        (spec.spec_tool_no_input ?? "").toString().trim();

      if (need && !val) {
        missingInput.push(`${spec.tool_no} : ${spec.spec_tool_no || "-"}`);
      }
    ...
    if (missingInput.length > 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณากรอกค่า Spec",
        html: `
      พบสเปคที่ยังไม่ได้กรอกค่า (Production Check / QC Line Check):<br/>
      <div style="text-align:left;margin-top:8px">
        ${missingInput.map((t) => `• ${t}`).join("<br/>")}
      </div>
    `,
      });
    ...
    // ✅ ตรวจว่ารายการที่เป็น Production Check / QC Line Check ต้องติ๊ก Pass หรือ Reject
    const MUST_CHECK = new Set(["Production Check", "QC Line Check"]);
    const missing = [];
    (specData || []).forEach((spec, idx) => {
      const sec = String(spec.section_check || "").trim();
      const need = MUST_CHECK.has(sec);
      const hasChoice = Boolean(passChecked?.[idx] || rejectChecked?.[idx]);
      if (need && !hasChoice) {
        // เก็บข้อความบอกตำแหน่งที่ยังไม่เลือก
        missing.push(`${spec.tool_no} : ${spec.spec_tool_no || "-"}`);
      }
    });
    ...
    if (missing.length > 0) {
      Swal.fire({
        ...
        title: "กรุณาเลือก Pass หรือ Reject",
        html: `
        พบสเปคที่ยังไม่ได้เลือกผล (Production Check / QC Line Check):<br/>
        <div style="text-align:left;margin-top:8px">
          ${missing.map((t) => `• ${t}`).join("<br/>")}
        </div>
      `,
      });
      return; // ❌ หยุดการบันทึก
    ...
    specData.forEach((spec, index) => {
      ...
      const updatedSpec = {
        ...
        end_time_qc_line: end_time_qc_line,
        section_check: spec.section_check,
        spec_tool_no: spec.spec_tool_no,
    ...
    try {
      ...
      if (
        ...
        // 🔶🔶 คำนวณ qcline_status_detail จากผลติ๊กทั้งหมด 🔶🔶
        const MUST_CHECK = new Set(["Production Check", "QC Line Check"]);
        const indices = (specData || [])
          .map((spec, idx) => ({
            idx,
            need: MUST_CHECK.has(String(spec.section_check || "").trim()),
          }))
          .filter((x) => x.need);

        const total = indices.length;
        const passCnt = indices.filter(
          ({ idx }) => Boolean(passChecked?.[idx]) && !Boolean(rejectChecked?.[idx])
        ).length;
        const rejectCnt = indices.filter(({ idx }) => Boolean(rejectChecked?.[idx])).length;

        let qclineStatusDetail; // undefined = ไม่ส่งฟิลด์นี้ถ้าให้คงค่าเดิม
        if (total > 0) {
          if (passCnt === total) qclineStatusDetail = "Pass";
          else if (rejectCnt > 0) qclineStatusDetail = "Reject";
        }

        const updateProductPayload = {
          productId: product.id,
          name_qc_by_off: nameQcCheck,
          end_time_qc_by_off: end_time_qc_line || null,
          // แนบเฉพาะเมื่อมีค่าที่สรุปได้
          ...(qclineStatusDetail ? { qcline_status_detail: qclineStatusDetail } : {}),
        };
        const updateProductResponse = await axios.put(
          config.api_path + "/product/updateToolAutoDetail",
  ...
  const handleSaveSpecQCLineOnly_Subcut_input_data = async (e) => {
    ...
    // ✅ ประกาศครั้งเดียว ใช้ร่วมกันทั้งฟังก์ชัน
    const MUST_SECTIONS = new Set(["Production Check", "QC Line Check"]);

    // -- ต้องกรอกค่า input
    const missingInput = [];
    (specData || []).forEach((spec, idx) => {
      const need = MUST_SECTIONS.has(String(spec.section_check || "").trim());
      const val =
        (inputValues?.[idx] ?? "").toString().trim() ||
        (spec.spec_tool_no_input ?? "").toString().trim();
      if (need && !val) {
    ...
    // -- ต้องเลือก Pass/Reject
    const missingCheck = [];
    (specData || []).forEach((spec, idx) => {
      const need = MUST_SECTIONS.has(String(spec.section_check || "").trim());
      const hasChoice = Boolean(passChecked?.[idx] || rejectChecked?.[idx]);
      if (need && !hasChoice) {
        missingCheck.push(`${spec.tool_no} : ${spec.spec_tool_no || "-"}`);
      }
    });
    ...
    // -- ตรวจความสอดคล้องระหว่างผลสรุป OK/NG กับผลรายรายการ
    if (!qclineStatus) {
      Swal.fire({ icon: "warning", title: "กรุณาเลือกสถานะสรุป (OK / NG)" });
      return;
    }
    ...
    const indices = (specData || [])
      .map((spec, idx) => ({
        idx,
        need: MUST_SECTIONS.has(String(spec.section_check || "").trim()),
      }))
      .filter((x) => x.need);
    ...
    // เกณฑ์:
    // OK → ต้องไม่มี Reject เลย
    // NG → ต้องมี Reject อย่างน้อย 1
    if (qclineStatus === "OK" && rejectCnt > 0) {
      Swal.fire({
        icon: "warning",
        title: "ผลสรุปเป็น OK แต่มีรายการที่ติ๊ก Reject",
        html: `โปรดตรวจสอบรายการที่ติ๊ก Reject ให้เป็น Pass หรือเปลี่ยนสถานะสรุปเป็น NG`,
      });
      return;
    }
    if (qclineStatus === "NG" && rejectCnt === 0) {
      Swal.fire({
        icon: "warning",
        title: "ผลสรุปเป็น NG แต่ไม่มีรายการที่ติ๊ก Reject",
        html: `กรุณาตรวจสอบอีกครั้ง (หากทุกแถวเป็น Pass ควรสรุปเป็น OK)`,
      });
      return;
    }
    ...
    specData.forEach((spec, index) => {
      ...
      const updatedSpec = {
        ...
        end_time_qc_line: end_time_qc_line || end_time_qc_lineAuto,  // 👈 แก้ตรงนี้
        section_check: spec.section_check,
        spec_tool_no: spec.spec_tool_no,
        spec_tool_no_input: inputValues[index] || spec.spec_tool_no_input,
        pass: passValue,
    ...
    try {
      ...
      if (
        ...
        const updateProductPayload = {
          productId: product.id,
          name_qc_by_off: nameQcCheck,
          // end_time_qc_by_off: end_time_qc_lineAuto || null,
          end_time_qc_by_off: end_time_qc_line || end_time_qc_lineAuto || null, // 👈 ใช้ค่าที่ผู้ใช้เปลี่ยน
  ...
  // true ถ้ามีสเปกอย่างน้อย 1 แถว
  const hasAnySpec = (specData || []).some(
    s => String(s?.spec_tool_no ?? "").trim() !== ""
  );
    ...
    <>
        ...
        <div className="signup_form w-50">
          <div className="card card-outline card-pink">
            <div className="card-header text-center" id="record-change-tool">
              <h3 className="h3">
                <b className="ml-3">
                  SEARCH DATA HISTORY ( SHAFT ) {Math.floor(timeLeft / 60)}:
                  {timeLeft % 60 < 10 ? "0" : ""}
                  {timeLeft % 60}
                </b>
                  ...
                  <div className="col-6">
                    <button
                      ...
                      onClick={() => {
                        ...
                        // ✅ ถ้าไม่ได้เลือก Machine ให้ส่ง null ไป
                        const machineValue = selectedOption ? selectedOption.value : null;
                        fetchDataSearch(machineValue);
                      }}
                    >
                      SEARCH MACHINE
                    </button>
            ...
            ? filteredProducts.map((item, index) => (
              <tr>
                ...
                <td className="text-center">
                  {(passChecked[index] || false) &&
                    !(rejectChecked[index] || false) ? (
                    <span>QC Line OK</span> // เมื่อ Pass Checked เป็นจริงและ Reject Checked เป็น false
                  ) : (
                    <>{item.machine}</> // ถ้าไม่ตรงเงื่อนไขให้แสดงค่าเครื่องจักร
                  )}
                </td>
                <td>{item.model}</td>
                <td>{item.process}</td>
                ...
                <td
                  className={`${item.qcline_status === "OK"
                    ...
                    : item.qcline_status === "NG"
                      ? "bg-danger text-center text-white"
                      : "text-center"
                    }`}
                >
                  ...
                  <br />
                  <span
                    className={item.qcline_status_detail === "Reject" ? "text-danger fw-bold" : "text-primary fw-bold"}
                  >
                    {item.qcline_status_detail}
                  </span>
                </td>

                {/* {productImages.find((image) => image.productId === product.id)?.status || 'N/A'} */}
                <td className="text-center">
                  {item.barcode === "Pass" &&
      ...
      {showModal && (
        <Modal id="modalProductTool" title="" modalSize="modal-lg">
          ...
          <form onSubmit={handleSave}>{/* ฟอร์มอื่น ๆ */}</form>
        </Modal>
      )}


      {/*------ Start Add Modal Register ---------------------------------------------------- */}
      <Modal id="modalProductShaft" title="" modalSize="modal-lg">
        <div className="col-12 mb-1" id="tool-production-list-p">
          <h3 className="h3">
            <b className="ml-3">RECORD TOOL DETAIL</b>
          </h3>
          ...
          <div className="row">
            ...
            <div className="mt-3 col-3">
              ...
              <datalist id="data_name">
                <option>ADISORN</option>
                <option>BOONSUEB</option>
                <option>BORDIN</option>
                <option>CHAKIT</option>
                <option>CHAKKIT</option>
                <option>CHAYUT</option>
                <option>KRITSADA</option>
                <option>NATTHANAN</option>
                <option>NIRUN</option>
                <option>POLLAPAT</option>
                <option>PRATOMPONG</option>
                <option>RATTANAPONG</option>
                <option>SARAYUT</option>
                <option>SURAPHAT</option>
                <option>TEERAPOP</option>
                <option>THANAPON</option>
                <option>WANCHAI</option>
              </datalist>
            </div>
              ...
              <form>
                <div className="card-body">
                  <div className="tool-numbers">
                    ...
                    {/* ✅ helper: เช็คว่า T17 ถูกเลือกอยู่หรือไม่ */}
                    {(() => {
                      const t17Selected =
                        Array.isArray(toolNumbers) &&
                        toolNumbers.some(
                          (t) => /^(t0?17)$/i.test(String(t.tool_no)) && t.selected
                        );

                      return toolNumbers.map((tool, toolIndex) => {
                        const isRough = /^(rough)$/i.test(String(tool.tool_no));

                        // ✅ ถ้า T17 ถูกเลือก และอันนี้คือ Rough → ซ่อนทั้งบล็อก
                        if (t17Selected && isRough) return null;

                        return (
                          <React.Fragment key={`${tool.tool_no}-${toolIndex}`}>
                            <div className="form-check tool-item">
                              <div className="tool-checkbox">
                                <input
                                  type="checkbox"
                                  className="form-check-input"
                                  id={`tool-${tool.tool_no}`}
                                  // Rough จะติ๊กอัตโนมัติ (ยกเว้นถูกซ่อนด้วย T17)
                                  checked={Boolean(tool.selected || isRough)}
                                  onChange={() => handleToolSelectionToolNumber(toolIndex)}
                                />
                                <label
                              ...
                              {/* แสดงปุ่มสถานะเมื่อเลือก หรือเป็น Rough (ถ้าเป็น Rough ซ่อนปุ่มสถานะตามกติกาก่อนหน้า) */}
                              {(tool.selected || isRough) && (
                                    ...
                                    <>
                                      <div>
                                        <input
                                          ...
                                          onChange={() =>
                                            handleStatusChangeToolNumber(toolIndex, "complete")
                                          }
                                          className="form-check-input"
                                        />
                                        <label
                                      ...
                                      <div>
                                        <input
                                          ...
                                          onChange={() =>
                                            handleStatusChangeToolNumber(toolIndex, "burr")
                                          }
                                          className="form-check-input"
                                        />
                                        <label id="tool-burr" htmlFor={`burr-${tool.tool_no}`}>
                                          Burr
                                        </label>
                                      </div>
                                      <div>
                                        <input
                                          ...
                                          onChange={() =>
                                            handleStatusChangeToolNumber(toolIndex, "broken")
                                          }
                                          className="form-check-input"
                                        />
                                        <label
                                      ...
                                      <div>
                                        <input
                                          ...
                                          onChange={() =>
                                            handleStatusChangeToolNumber(toolIndex, "cf")
                                          }
                                          className="form-check-input"
                                        />
                                        <label id="tool-cf" htmlFor={`cf-${tool.tool_no}`}>
                                          C/F
                                        </label>
                                      </div>
                                    </>
                            ...
                            {/* แสดง specs เมื่อเลือก หรือเป็น Rough (Rough จะถูกซ่อนถ้า T17 ถูกเลือก) */}
                            {(tool.selected || isRough) && (
                              <div className="specs-container mt-3">
                                {tool.specs.map((spec, specIndex) => {
                                  const isQCLineCheck = spec.section_check === "QC Line Check";

                                  return (
                                    <div
                                      key={`${tool.tool_no}-${spec.spec_tool_no || specIndex}`}
                                      className={`spec-item ${isQCLineCheck ? "highlight" : ""}`}
                                    >
                                      <p>
                                        <span className="underline">Tool :</span> {spec.tool_no}
                                        <span className="spacing"></span>
                                        <span className="underline">Part no :</span> {spec.part_no}
                                        <span className="spacing"></span>
                                        <span className="underline">Rev :</span> {spec.rev_control}
                                        <span className="spacing"></span>
                                        <span className="underline">Section :</span> {spec.section_check}
                                        <br />
                                        <span style={{ color: "blue" }}>
                                          <span className="underline">Spec :</span> {spec.spec_tool_no}
                                        </span>
                                      </p>
                                      ...
                                      <div className="input-checkbox-container d-flex align-items-center">
                                        <input
                                          type="text"
                                          value={specInputs[toolIndex]?.[specIndex] || ""}
                                          onChange={(e) =>
                                            handleSpecInputChange(toolIndex, specIndex, e)
                                          }
            ...
            <div className="card-body" id="bodyRoncom">
              ...
              <div className="card">
                <div className="row ml-3 mt-2">
                  ...
                  <div className="col-sm-6 ml-2 mt-1">
                    <div className="form-group clearfix">
                      <div className="d-inline">
                        <input
                          type="checkbox"
                          className="large-checkbox"
                          checked={checkboxValues.sensor_check_tap}
                          onChange={() => handleCheckboxChangeAll("sensor_check_tap")}
                          id="checkboxPrimarysensor_check_tap"
                        />
                        <label className="" htmlFor="sensor_check_tap">
                          Sensor Check Tap ( เมือเปลี่ยน Drill หรือ Tap)
                        </label>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="form-group clearfix">
                      <div className="d-inline">
                        <input
                          type="checkbox"
                          className="large-checkbox"
                          checked={checkboxValues.sensor_check_1}
                          onChange={() => handleCheckboxChangeAll("sensor_check_1")}
                          id="checkboxPrimarysensor_check_1"
                        />
                        <label className="" htmlFor="sensor_check_1">
                          Sensor Check Part หัวตัด
                        </label>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="form-group clearfix">
                      <div className="d-inline">
                        <input
                          type="checkbox"
                          className="large-checkbox"
                          checked={checkboxValues.sensor_check_2}
                          onChange={() => handleCheckboxChangeAll("sensor_check_2")}
                          id="checkboxPrimarysensor_check_2"
                        />
                        <label className="" htmlFor="sensor_check_2">
                          Sensor Check Part หัวตัดและดักงาน (เฉพาะ SDH)
                        </label>
                      </div>
                    </div>
                  </div>
        ...
        <div className="row">
          ...
          <div className="row">
            ...
            <div className="col-6 mt-1">
              ...
              <input
                value={product.sensor_check_tap}
                className="form-control text-primary pr-1"
              />
            </div>
            ...
            {!product.nameafterset && (
              <>
                <input
                  ...
                  placeholder="Name..."
                />
                <datalist id="data_name"></datalist>
              </>
            )}
          </div>
          <div className="col-2 mt-2 ml-2">
              ...
              <div className="col-12" key={item.id}>
                ...
                <div>
                  <iframe
                    ...
                    src={config.api_path + "/uploads/" + item.imageName} title="Preview"></iframe>
                </div>
                <div>{/* {" "} */}</div>
              </div>
            ))
      ...
      <ModalQCInprocessShaft id="modalContourOverTarget" modalSize="modal-lg">
        <h3 className="h3 qc-line-check">
          <b className="ml-3">QC Line Check</b>
        </h3>
        <div className="row">
          <div className="col-2">
            <div className="text-bold" id="box-product">
        ...
        <div className="row">
          {Object.entries(product).map(([key, value], index) => {
            if (/^t\d+$/.test(key) && value) {
              const isRoughCol = key.toLowerCase() === "t43";
              return (
                  ...
                  <div className="form-group clearfix">
                    <div className="d-inline">
                      <input
                        ...
                        onChange={(e) => handleInputChangeContourOver(e, key, "Over")}
                        className="form-control text-primary"
                      />
                      <label
                      ...
                      >
                        {isRoughCol ? "ROUGH" : key.toUpperCase()}
                      </label>
                    </div>
                  </div>
                </div>
        ...
        <div className="row">
          ...
          <div className="col-3">
            ...
            <input
              value={end_time_qc_line}
              onChange={(e) => setEnd_time_qc_line(e.target.value)}
              className="form-control text-primary mb-2"
          ...
          <div className="col-6 mt-2">
            ...
            <input
              type="checkbox"
              className="mr-2" id="checkboxQCLine"
              checked={showSaveButton}
              onChange={(e) => setShowSaveButton(e.target.checked)}
            />

            {/* ✅ แสดงปุ่มเฉพาะเมื่อ checkbox ถูกติ๊ก */}
            {showSaveButton && (
              <button
                className="btn btn-success mt-2"
                onClick={handleSaveQCBYoffTNNew}
              >
                Save new Time Start
              </button>
            )}
        ...
        {product.time_qc_by_off ? (
          <>
            {(() => {
              // 🔹 จัดกลุ่ม spec ตาม tool_no และเก็บ index เดิมไว้ใช้กับ state ต่าง ๆ
              const groupedByTool = specData.reduce((acc, spec, idx) => {
                const key = String(spec.tool_no || "").trim();
                if (!acc[key]) acc[key] = [];
                acc[key].push({ spec, idx });
                return acc;
              }, {});

              return Object.entries(groupedByTool).map(([toolNo, items]) => {

                return (
                  <div key={toolNo} className="col-12 mb-3">
                    {/* หัวข้อของแต่ละ Tool */}
                    <div
                      className="py-2 px-3"
                      style={{
                    ...
                    >
                      {items.map(({ spec, idx }, rowIndex) => (
                        <div
                          key={`${toolNo}-${spec.spec_tool_no || rowIndex}`}
                          className="row align-items-center py-2"
                          style={{
                            borderBottom:
                              rowIndex < items.length ? "1px dashed #212121ff" : "none",
                          }}
                        >
                          <div
                            className="col-5"
                            style={{
                              backgroundColor:
                                spec.section_check === "QC Line Check" ? "yellow" : "",
                              fontWeight: "bold",
                              padding: "10px",
                              borderRadius: "5px",
                            }}
                          ...
                          <div className="col-3">
                            <input
                              type="text"
                              value={inputValues[idx] || spec.spec_tool_no_input || ""}
                          ...
                          <div className="col-2 d-flex align-items-center">
                            <input
                              className="form-check-input pass-checkbox-qcline ml-1"
                              type="checkbox"
                              checked={passChecked[idx] || false}
                              onChange={() => handleCheckboxChangePassReject(idx, "pass")}
                            />
                            <span className="ml-5" id="qc-line-pass">Pass</span>
                          </div>

                          <div className="col-2 d-flex align-items-center">
                            <input
                              className="form-check-input reject-checkbox-qcline ml-1"
                              type="checkbox"
                              checked={rejectChecked[idx] || false}
                              onChange={() => handleCheckboxChangePassReject(idx, "reject")}
                            />
                            <span className="ml-5" id="qc-line-pass">Reject</span>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {/*--- Start Save Tool no spec to EQM   ------------------------*/}

        {product.time_qc_by_off && product.area_qc_check === "QC Eqm" && noSpec && (
          <>
            <div className="row">
              <div className="col-4 mt-2 mb-4 ml-1">
                <div
                  className="text-bold text-center"
                  id="qc-inprocess-confirm"
                >
                  Input Name QC Check to EQM
                </div>
                <input
                  type="text"
                  className="form-control text-primary mb-1"
                  onChange={(e) => setNameQcCheck(e.target.value)}
                  placeholder="Input ............."
            ...
            <button
              ...
              onClick={SaveNameQCLine_To_Qc_Eqm}
            >
              Save Tool to EQM
            </button>
          </>
        ...
        {product.area_qc_check === "QC Line" &&
          ...
          (
            <>
              <div className="row">
                <div className="col-6 mt-2 mb-4 ml-1">
                  <div
                    className="text-bold"
                    id="qc-inprocess-confirm"
                  >
                    Input Name QC Check Finish {" >> "} {product.name_qc_by_off}
                  </div>
                  <input
                    type="text"
                    className="form-control text-primary mb-2"
                ...
                <div className="col-3 mt-2 mb-4 ml-1">
                  <div
                    className={`text-center ${product.qcline_status === "OK"
                      ? "bg-success text-white"   // ✅ เขียว
                      : product.qcline_status === "NG"
                        ? "bg-danger text-white"   // ✅ แดง
                        : "bg-secondary text-white" // ✅ เทา (ไม่มีข้อมูล)
                      }`}
                  >
                    <span>Status {">>"} {product.qcline_status}</span>
                  </div>
                  <select
                    id="statusValue"
                    className="form-control"
                    value={qclineStatus}
                    onChange={handleQclineStatusChange}
                  >
...
export default ToolNumberSearchShaft;
