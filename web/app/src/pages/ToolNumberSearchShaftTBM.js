...
function ToolNumberSearchShaftTBM(props) {
  ...
  const fetchDataSearch = async (machine) => {
    try {
      ...
      const response = await axios.post(
        config.api_path + "/product/ProductMCQC_TBM",
  ...
  const handlePageClick = async (data) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/product/listNewMachineShaftTBM?offset=${offset}&limit=${limit}`,
        config.headers()
      );
  ...
  const getMachines = async () => {
    try {
      const response = await axios.get(config.api_path + "/machinesTBM/listShaft");
  ...
  const getMachinesSearch = async () => {
    try {
      const response = await axios.get(config.api_path + "/getDataMCTBMSearch");
  ...
  const handleMachineSelect = async (selectedOption) => {
    ...
    try {
      ...
      if (response.data.message === "success") {
        ...
        handleOkClickToolNumberListTBM();
      } else {
        Swal.fire({
          title: "error",
  ...
  const handleOkClickContourOverTarget = async () => {
    try {
      const tNumbers = Object.keys(selectedOverTarget)
        .filter((t) => selectedOverTarget[t] && selectedOverTarget[t] !== "")
        .map((t) => {
          const numberPart = t.replace(/^T/i, "");
          return "T" + numberPart.padStart(2, "0");
        });
      const machineNumber = product.machine;
      const response = await axios.post(
  ...
  const handleCheckboxChangePassReject = (index, type) => {
    ...
    if (type === "pass") {
      ...
      // console.log(`Pass changed: ${updatedSpecData[index].pass}, Reject: ${updatedSpecData[index].reject}`); // Log ตรวจสอบค่าของ pass และ reject
      setSpecData(updatedSpecData);
    } else if (type === "reject") {
      updatedSpecData[index].reject =
        updatedSpecData[index].reject === "Yes" ? "No" : "Yes";
      updatedSpecData[index].pass = "No";
      // console.log(`Reject changed: ${updatedSpecData[index].reject}, Pass: ${updatedSpecData[index].pass}`); // Log ตรวจสอบค่าของ reject และ pass
      setSpecData(updatedSpecData);
    }
  };
  ...
  const handleOkClickToolNumberListTBM = async () => {
    try {
      ...
      const response = await axios.post(
        `${config.api_path}/api/toolNumBerListNewSpecTBM`,
        {
          machineNumber: machineNumber,
  ...
  return (
    <>
            ...
            <div className="card-body" id="bodyRoncom">
              <div className="container" id="tool-number">
                <button
                  ...
                  id="toolNumber-auto"
                  type="button"
                  onClick={handleOkClickToolNumberListTBM}
                >
                  SELECT TOOL NUMBER
                </button>
              </div>
                  ...
                  <div className="tool-numbers">
                    ...
                    {toolNumbers.map((tool, toolIndex) => (
                      <>
                        ...
                        {/* แสดง spec เฉพาะเมื่อ checkbox ถูกเลือก */}
                        {tool.selected && (
                          <div className="specs-container mt-3">
                            {tool.specs.map((spec, specIndex) => {
                              const isQCLineCheck =
                                spec.section_check === "QC Line Check"; // ตรวจสอบเงื่อนไข
                              return (
                                <div
                                  key={specIndex}
                                ...
                                >
                                  ...
                                  <div className="input-checkbox-container d-flex align-items-center">
                                    ...
                                    <input
                                      ...
                                      // className="form-control me-3 text-primary" // Add margin-right for spacing
      ...
      <ModalQCInprocessShaft id="modalContourOverTarget" modalSize="modal-lg">
        ...
        {product.time_qc_by_off ? (
          <>
            {(() => {
              ...
              // (ถ้าคุณต้องการซ่อน Rough เมื่อ T17 ถูกเลือก สามารถเปิดบรรทัดนี้)
              // const t17Selected = Array.isArray(toolNumbers) && toolNumbers.some(t => /^(t0?17)$/i.test(String(t.tool_no)) && t.selected);

              return Object.entries(groupedByTool).map(([toolNo, items]) => {
                // ถ้าต้องการซ่อน Rough เมื่อ T17 ถูกเลือก ให้ใช้ guard นี้:
                // if (t17Selected && /^rough$/i.test(toolNo)) return null;
                ...
                return (
                  <div key={toolNo} className="col-12 mb-3">
                    ...
                    {/* กล่องรายละเอียดของสเปคทั้งหมดภายใน Tool เดียวกัน */}
                    <div
                      style={{
                        border: "2px solid #ccc",
                        borderTop: "none",
        ...
        {/* {nameQcLine && ( // ถ้า nameQcLine มีค่า (null หรือว่าง) แสดงปุ่ม Update
          <button className="btn btn-success mb-3 ml-1" onClick={handleSaveSpecQCLine}>
            Update Spec
          </button>
        )} */}
        <hr />
...
export default ToolNumberSearchShaftTBM;
