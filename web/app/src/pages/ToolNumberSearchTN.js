...
function ToolNumberSearchTN(props) {
  ...
  const fetchDataSearch = async (machine) => {
    try {
      ...
      const response = await axios.post(
        config.api_path + "/product/ProductMCQC_TN",
  ...
  const handlePageClick = async (data) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/product/listNewProductionSleeveTN?offset=${offset}&limit=${limit}`,
        config.headers()
      );
  ...
  const getMachines = async () => {
    try {
      const response = await axios.get(config.api_path + "/machinesTN/listOnlyTool");
  ...
  const getMachinesSearch = async () => {
    try {
      const response = await axios.get(config.api_path + "/getDataMCTNSearch");
  ...
  const getNameTN = async () => {
    try {
      const response = await axios.get(config.api_path + "/getNameTN");
      setGetNameTn(response.data.results || []); // ✅ set เฉพาะ results
    } catch (error) {
      console.error("Error fetching machines:", error);
    }
  };
 ...
 const handleSaveCancelRecord = async (e) => {
  ...
  const handleOkClickToolNumberList = async () => {
    try {
      ...
      const response = await axios.post(
        `${config.api_path}/api/toolNumBerListNewSpecSleeveTN`,
        {
          machineNumber: machineNumber,
  ...
  const handleSaveNewAuto = async (e) => {
    ...
    // 🔹 เช็คว่าผู้ใช้งานเลือก NAME หรือยัง
    if (!product.name || product.name.trim() === "") {
      Swal.fire({
        title: "กรุณาเลือกชื่อ",
        text: "กรุณาเลือกชื่อผู้ใช้งาน NAME",
        icon: "warning",
      });
      return;
    }

    // 🔹 เงื่อนไขใหม่: ต้องกด SELECT TOOL NUMBER ก่อน
    if (!toolNumbersFetched || toolNumbers.length === 0) {
      Swal.fire({
        title: "กรุณาเลือก Tool Number",
        text: "กรุณากดปุ่ม SELECT TOOL NUMBER เพื่อเลือก Tool no.",
        icon: "warning",
      });
      return;
    }
  ...
  const handleSaveSpecQCLine_TN_input_data = async (e) => {
    ...
    try {
      ...
      if (
        ...
        const updateProductResponse = await axios.put(
          config.api_path + "/productTN/TNupdateToolAutoSleeve",
          updateProductPayload,
          config.headers()
        );
        console.log(
  ...
  return (
    <>
        ...
        <form onSubmit={handleSaveNewAuto}>
          ...
          <div className="row">
            ...
            <div className="mt-3 col-3">
              ...
              <Select
                id="nameRegister"
                options={getNameTn.map((item) => ({
                  value: item.name.toUpperCase(),   // ✅ บังคับเป็นพิมพ์ใหญ่
                ...
                value={
                  getNameTn
                    ...
                    .find((opt) => opt.value === product.name) || null
                }
                onChange={(selectedOption) =>
                  setProduct({
                    ...product,
            ...
            <div className=" main1 mt-3 col-3">
              ...
              <input
                onChange={(e) =>
                  setProduct({ ...product, shift: e.target.value.toUpperCase(), })
                }
                list="data101"
                className="form-control fw-bold"
                id="shiftRegister"
              ...
              <Select
                ...
                value={
                  getNameTn
                    ...
                    .find((opt) => opt.value === nameafterset) || null
                }
                onChange={(selectedOption) =>
                  setNameafterset(selectedOption ? selectedOption.value : "")
                }
                placeholder="Name.."
                isSearchable={true} // ✅ ค้นหาได้
              />
            )}
          </div>
          <div className="col-2 mt-2 ml-2">
              ...
              <Select
                ...
                value={
                  getNameTn
                    ...
                    .find((opt) => opt.value === nameafterset2) || null
                }
                onChange={(selectedOption) =>
                  setNameafterset2(selectedOption ? selectedOption.value : "")
                }
                placeholder="Name.."
                isSearchable={true} // ✅ ค้นหาได้
              />
              ...
              <Select
                ...
                value={
                  getNameTn
                    ...
                    .find((opt) => opt.value === nameafterset3) || null
                }
                onChange={(selectedOption) =>
                  setNameafterset3(selectedOption ? selectedOption.value : "")
                }
                placeholder="Name.."
                isSearchable={true} // ✅ ค้นหาได้
              />
              ...
              <Select
                ...
                value={
                  getNameTn
                    ...
                    .find((opt) => opt.value === nameafterset4) || null
                }
                onChange={(selectedOption) =>
                  setNameafterset4(selectedOption ? selectedOption.value : "")
                }
                placeholder="Name.."
                isSearchable={true} // ✅ ค้นหาได้
              />
              ...
              <Select
                ...
                value={
                  getNameTn
                    ...
                    .find((opt) => opt.value === nameafterset5) || null
                }
                onChange={(selectedOption) =>
                  setNameafterset5(selectedOption ? selectedOption.value : "")
                }
                placeholder="Name.."
                isSearchable={true} // ✅ ค้นหาได้
              />
      ...
      <ModalQCInprocessShaft id="modalContourOverTarget" modalSize="modal-lg">
        ...
        {/*---------  ปิด Input Name QC Line? โดยเปลี่ยนจาก !product.time_qc_by_off เป็น product.time_qc_by_off ---------*/}
        ...
        {/* {hasTNMachine && product.area_qc_check === "QC Line" && products.time_qc_by_off && (
          <button className="btn btn-success mb-3 ml-1" onClick={handleSaveSpecQCLineOnly_TN_input_data}>
            Save Name QC Line TN?
          </button>
        )}
         */}
...
export default ToolNumberSearchTN;
