...
function AddPdfMachineTBMAllModelInput() {
    const [productImage, setProductImage] = useState({});
    const [productImages, setProductImages] = useState([]);
    const [process, setProcess] = useState('');
    const [model, setModel] = useState('');
    const [toolnumber, setToolnumber] = useState('');
    const [machine_type, setMachine_type] = useState('TBM');
        ...
        Swal.fire({
            ...
            if (res.isConfirmed) {
                try {
                    ...
                    formData.append("machine_type", "TBM");

                    await axios
                        .post(config.api_path + "/productImage/insertAllModelTBMMachine", formData, _config)
                        .then((res) => {
    ...
    const fetchDataProductImage = async () => {
        try {
            const response = await axios.get(config.api_path + "/productImage/getAllModelTBMMachine");
    ...
    const handleDelete = (item) => {
        ...
        Swal.fire({
            ...
            if (res.isConfirmed) {
                try {
                    await axios.delete(config.api_path + '/productImage/deleteAllModelTBMMachine/' + item.id, config.headers()).then(res => {
                        if (res.data.message === 'success') {
                            Swal.fire({
                                title: 'ลบข้อมูล',
                                text: 'ลบข้อมูลแล้ว',
        ...
        Swal.fire({
            ...
            if (res.isConfirmed) {
                try {
                    ...
                    await axios
                        .put(config.api_path + `/productImage/updateAllModelTBMMachine/${selectedItem.id}`, formData, _config)
                        .then((res) => {
                            if (res.data.message === "success") {
                                Swal.fire({
                                    title: "Update Profile",
    ...
    useEffect(() => {
        if (selectedItem && selectedItem.id) { // ตรวจสอบว่ามี selectedItem และมี id
          const fetchPdfData = async () => {
            try {
              const response = await axios.get(`${config.api_path}/productImage/getUpdateTBMImage/${selectedItem.id}`);
              if (response.data.message === 'success') {
        ...
        <>
                            ...
                            <div className="card-header text-center">
                                <div className="card">
                                    <div className="card-header">
                                        <h3>
                                            <b className="fw-bold">ADD RECORD PDF ALL MODEL TBM MACHINE</b>
                                        </h3>
                                    </div>
            ...
            <Modal id="modalProductImage" title="" modalSize="modal-lg">
                <div className="col-12 mb-3" id="add-record-master-pdf">
                    <h3 className="h3">
                        <b className="ml-3">ADD RECORD MASTER PDF ALL MODEL TBM MACHINE</b>
                    </h3>
                </div>
                <div className="row">
            ...
            <Modal id="modalShowProductImage" title="" modalSize="modal-lg">
                <div className="col-12 mb-3" id="add-record-master-pdf">
                    <h3 className="h3">
                        <b className="ml-3">RECORD MASTER PDF ALL TBM MACHINE</b>
                    </h3>
                </div>
                <hr className="mt-5">
                </hr>
            ...
            <Modal id="modalProductImageUpdate" title="" modalSize="modal-lg">
                <div className="col-12 mb-3" id="add-record-master-pdf">
                    <h3 className="h3">
                        <b className="ml-3">UPDATE RECORD MASTER PDF ALL MODEL TBM MACHINE</b>
                    </h3>
                </div>
                <div className="row">
                    <div className="col-2 mt-3">
...
export default AddPdfMachineTBMAllModelInput;