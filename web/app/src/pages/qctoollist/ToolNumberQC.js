...
const playAnnouncement = (machine, status) => {
  ...
  const audioFilePath = `${process.env.PUBLIC_URL}/audio/ch154/${machine} ${status}.${fileExtension}`;
...
function ToolNumberQC() {
  ...
  const handleSaveStatusToProduct = async (e) => {
    ...
    try {
      ...
      if (response.data.message === 'success') {
        ...
        // Send data to machine using /machine/machineRegisterIP
        const machinePayload = {
          ip_address: product.ip_address, // ใส่ค่าที่เกี่ยวข้อง
          system_no: product.system_no, // ใส่ค่าที่เกี่ยวข้อง
          address_type: product.address_type, // ใส่ค่าที่เกี่ยวข้อง
          address_no: product.address_no // ใส่ค่าที่เกี่ยวข้อง
        };

        console.log('Payload to machine:', machinePayload);

        try {
          const machineResponse = await axios.post(
            config.api_path + '/machine/machineRegisterIP',
            machinePayload,
            config.headers()
          );
          ...
          if (machineResponse.status === 200) {
            Swal.fire({
              title: 'แจ้งสถานะสำเร็จ',
              text: machineResponse.data.message || 'ส่งสถานะไฟสำเร็จแล้ว',
              icon: 'success',
              timer: 5000
            });
          } else {
            Swal.fire({
              ...
              text: machineResponse.data.message || 'เกิดข้อผิดพลาดในการส่งสถานะไฟ',
              icon: 'warning',
              timer: 5000
            });

            // ออกจากฟังก์ชันโดยไม่กระทบ API เส้นแรก
            return;
          }
        } catch (machineError) {
          console.error('Error while sending data to machine:', machineError);

          Swal.fire({
            title: 'แจ้งสถานะไม่สำเร็จ',
            text: machineError.response?.data?.message || 'ไม่สามารถส่งสถานะไปยังเครื่องจักรได้',
            icon: 'error',
            timer: 5000
          });
        ...
        // Reload and fetch data if machine status is successful
        window.location.reload();
        fetchData();
      }
    } catch (e) {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${config.api_path}/product2/listMachineQCSleeve?page=${currentPage}&limit=${itemsPerPage}`,
        config.headers()
      );
      if (response.data.message === "success") {
        setProducts(response.data.results);
  ...
  const handleMCTypeChange = (value) => {
    ...
    if (value === "CH") {
      navigate("/toolNumberQC");
      window.location.reload();
    } else if (value === "CS") {
      navigate("/toolNumberQCCS");
      window.location.reload();
    } else if (value === "SB") {
      navigate("/toolNumberQCSB");
      window.location.reload();
    } else if (value === "TN") {
      navigate("/toolNumberQCTN");
      window.location.reload();
    } else {
  ...
  socket.on("productAdded", (newProduct) => {
    console.log("🆕 Product inserted via socket:", newProduct);
    setProducts((prev) => [newProduct, ...prev]); // เพิ่มรายการใหม่บนสุด
  });
  ...
  const handleSaveAfterSet_4_Production = async (e) => {
    ...
    const result = await Swal.fire({
      title: "ยืนยันการบันทึก?",
      text: "คุณต้องการบันทึกสถานะ AF4 ใช่หรือไม่",
      icon: "question",
      showCancelButton: true,
      confirmButtonText: "ยืนยัน",
      cancelButtonText: "ยกเลิก",
  ...
  return (
    <>
        ...
        <div className="signup_form w-50">
          <div className="card card-outline card-dark">
            <div className="card-header text-center" id="product-tool-list">
              <h3 className="h3">
                <b className="ml-3">
                  SEARCH BARCODE PRODUCTION CHANGE TOOL ( SLEEVE ){" "}
      ...
      <table
        ...
        id="table-qc-search" style={{ fontSize: "1.0rem" }}
      >
      ...
      <Modal id="modalCancelRecord" title="" modalSize="modal-lg">
