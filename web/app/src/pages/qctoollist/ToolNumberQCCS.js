  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${config.api_path}/product2CS/listMachineQCSleeveCS?page=${currentPage}&limit=${itemsPerPage}`,
        config.headers()
      );
      if (response.data.message === "success") {
        setProducts(response.data.results);
                ...
                {userSelectedContour === "NG(Drawing)" && (
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setProduct((prev) => ({...prev,contour_ng_target_spec: e.target.value.toUpperCase(),
                        }))
                      }
                      type="text"
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setProduct((prev) => ({...prev,contour_ng_drawing_spec: e.target.value,
                        }))
                      }
                      type="text"
                      className="form-control mt-2"
