...
function ToolNumberQCShaftTB() {
 ...
 const fetchData = async () => {
  try {
    let machineFilter = ""; // ตัวแปรใช้เก็บค่า filter

    if (selectedMCType === "TB") {
      machineFilter = "TB-%";  // ดึงข้อมูลเฉพาะที่เริ่มต้นด้วย "TB-"
    } else if (selectedMCType === "") {
      machineFilter = "";  // หรือดึงข้อมูลทั้งหมดหากไม่ได้เลือกค่า
    }

    const response = await axios.get(
      `${config.api_path}/product2TB/listMachineQCShaftTB?page=${currentPage}&limit=${itemsPerPage}&machineFilter=${machineFilter}`,
      config.headers()
    );
    ...
    if (response.data.message === "success") {
      ...
      setTotalPages(Math.ceil(Math.min(response.data.totalItems, 200) / itemsPerPage));
    }
  } catch (error) {
    Swal.fire({
      title: "Error",
...
export default ToolNumberQCShaftTB;
