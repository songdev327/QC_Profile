  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${config.api_path}/product2TN/listMachineQCSleeveTN?page=${currentPage}&limit=${itemsPerPage}`,
        config.headers()
      );
      if (response.data.message === "success") {
        setProducts(response.data.results);
      ...
      <div className="row">
        <div className="col-12 items-per-page-containerSearch mb-3">
          ...
          <div className="btn-group" role="group" aria-label="MC Types">
            ...
            <button
              type="button"
              // className={`btn ${selectedMCType === "CS" ? "btn-primary" : "btn-light"}`}
              className={`btn ${selectedMCType === "CS" ? "btn-primary" : "btn-light"} ml-4`} // เพิ่ม me-2
