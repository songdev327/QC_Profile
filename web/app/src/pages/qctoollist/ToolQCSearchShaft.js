  ...
  const handlePageClick = async (data) => {
    ...
    try {
      const response = await axios.get(
        `${config.api_path}/productSFH/listNewComponentSFH?offset=${offset}&limit=${limit}`,
        config.headers()
      );
    ...
    <>
        ...
        <div className="signup_form w-50">
          <div className="card card-outline card-yellow">
            <div className="card-header text-center" id="tool-qc-search">
              <h3 className="h3">
                <b className="ml-3">
                  ALL SEARCH DATA HISTORY ( SHAFT ) {Math.floor(timeLeft / 60)}:
                  {timeLeft % 60 < 10 ? "0" : ""}
                  {timeLeft % 60}
                </b>
      ...
      <Modal id="modalProductImage" title="" modalSize="modal-lg">
        ...
        <div className="row">
          ...
          <div className="col-12 mb-1" id="tool-production-Afterset-qc">
            <h3 className="h3">
          ...
          {/* <div className="col-7 ml-3" id="box-1">
...
export default ToolQCSearchShaft;
