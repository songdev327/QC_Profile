...
function MachineCHForProduct() {
    ...
    const [machine_type, setMachine_type] = useState("CH");  // state สำหรับควบคุม
    ...
    const fetchDataProductImage = async () => {
        try {
            const response = await axios.post(config.api_path + "/productImage/searchCH", {
    ...
    const getToolNumber = async () => {
        try {
            const response = await axios.get(config.api_path + "/productImage/getToolNumberCH");
                ...
                <div className="content-wrapper">
                    <div className="container-fluid" id="list-master">
                        <h1 className="" style={{ marginLeft: "10px" }}>
                            CH List Master Record Change Tool PDF
                        </h1>
                    </div>
                    <hr />
...
export default MachineCHForProduct;
