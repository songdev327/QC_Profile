...
function MachineCSForProduct() {
    ...
    const [machine_type, setMachine_type] = useState("CS");  // state สำหรับควบคุม
    ...
    const fetchDataProductImage = async () => {
        try {
            const response = await axios.post(config.api_path + "/productImage/searchCS", {
    ...
    const getToolNumber = async () => {
        try {
            const response = await axios.get(config.api_path + "/productImage/getToolNumberCS");
                ...
                <div className="content-wrapper">
                    <div className="container-fluid" id="list-master">
                        <h1 className="" style={{ marginLeft: "10px" }}>
                            CS List Master Record Change Tool PDF
                        </h1>
                    </div>
                    <hr />
...
export default MachineCSForProduct;
