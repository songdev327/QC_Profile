...
function MachineTBForProduct() {
    ...
    const [machine_type, setMachine_type] = useState("TB");  // state สำหรับควบคุม
    ...
    const fetchDataProductImage = async () => {
        try {
            const response = await axios.post(config.api_path + "/productImage/searchTB", {
    ...
    const getToolNumber = async () => {
        try {
            const response = await axios.get(config.api_path + "/productImage/getToolNumberTB");
                ...
                <div className="content-wrapper">
                    <div className="container-fluid" id="list-master">
                        <h1 className="" style={{ marginLeft: "10px" }}>
                            TB List Master Record Change Tool PDF
                        </h1>
                    </div>
                    <hr />
...
export default MachineTBForProduct;