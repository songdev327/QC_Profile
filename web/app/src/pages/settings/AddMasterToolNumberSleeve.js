...
function AddMasterToolNumberSleeve() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/masterToolNumber/masterToolNumberInsertSleeve",
          payload
        );
        if (response.data.message === "success") {
          Swal.fire({
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(
        config.api_path + "/masterToolNumber/masterToolNumberListSleeve",
        config.headers()
      );
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios
            .put(
              config.api_path + "/masterToolNumber/updateSleeve/" + item.id,
              {
  ...
  const handleDelete = async (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(
            config.api_path + "/machinesToolNumber/deleteSleeve/" + item.id,
            config.headers()
          );
          Swal.fire({
            title: "ลบข้อมูล",
  ...
  const getToolNumberSearch = async () => {
    try {
      const response = await axios.get(
        config.api_path + "/machinesToolNumber/listSearchToolSleeve"
      );
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">ADD NEW MASTER TOOL NUMBER ( CH MACHINE )</b>
                </h3>
              </div>
...
export default AddMasterToolNumberSleeve;