...
function AddMasterToolNumberSpecCS() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/masterNumberSleeve/masterNumberSpecToolInsertCS",
          payload
        );
        if (response.data.message === "success") {
          Swal.fire({
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(
        config.api_path +
          "/masterNumberSleeve/masterNumberSpecNumberToolListCS",
        config.headers()
      );
      ...
      if (res.isConfirmed) {
        try {
          await axios
            .put(
              config.api_path +
                "/masterNumberSleeve/masterNumberSpecNumberToolupdateCS/" +
                item.id,
              {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(
            config.api_path +
              "/masterNumberSleeve/masterNumberSpecNumberTooldeleteCS/" +
              item.id,
            config.headers()
          );
          Swal.fire({
  ...
  const getToolNumberSearch = async () => {
    try {
      const response = await axios.get(
        config.api_path + "/masterSpecToolSleeve/masterSpecToolSearchSleeveCS"
      );
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${config.api_path}/upload-master-tool-spec-cs`,
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">
                    ADD NEW MASTER TOOL NUMBER SPEC ( CS MACHINE )
                  </b>
                </h3>
              </div>
...
export default AddMasterToolNumberSpecCS;
