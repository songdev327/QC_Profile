...
function AddMasterToolNumberSpecSleeve() {
  ...
  // ฟังก์ชันสำหรับสร้างไฟล์ Excel และดาวน์โหลด
  const exportToExcel = () => {
  ...
  const handleMasterSpecQcLine = async (e) => {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const payload = {
          Machine_Number: machineSearchTerm, // ใช้ machineSearchTerm แทน
          Partname_Model: modelSearchTerm, // ใช้ modelSearchTerm แทน
        ...
        const response = await axios.post(
          config.api_path + "/masterNumberSleeve/masterNumberSpecToolInsert",
          payload
        );
        if (response.data.message === "success") {
          Swal.fire({
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(
        config.api_path + "/masterNumberSleeve/masterNumberSpecNumberToolList",
        config.headers()
      );
      if (res.data.message === "success") {
        setMachineNumbers(res.data.results);
        console.log("Fetched Data:", res.data.results); // ตรวจสอบข้อมูลที่ได้รับ
      ...
      if (res.isConfirmed) {
        try {
          await axios
            .put(
              config.api_path +
              "/masterNumberSleeve/masterNumberSpecNumberToolupdate/" +
              item.id,
              {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(
            config.api_path +
            "/masterNumberSleeve/masterNumberSpecNumberTooldelete/" +
            item.id,
            config.headers()
          );
          Swal.fire({
  ...
  const getToolNumberSearch = async () => {
    try {
      const response = await axios.get(
        config.api_path + "/masterSpecToolSleeve/masterSpecToolSearchSleeve"
      );
  ...
  const handleSearch = () => {
    if (
      ...
      Swal.fire({
        title: "Error",
        text: "กรุณาเลือก Type Machine, Model หรือ Tool Number เพื่อค้นหา",
        icon: "error",
      });
      return;
    }
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">
                    ADD NEW MASTER TOOL NUMBER SPEC ( CH MACHINE )
                  </b>
                </h3>
              </div>
              <div className="card-body">
                <div className="row">
                  ...
                  <div className="col-12">
                    <div className="input-group">
                      {/* ใช้ Select จาก react-select แทน input */}
                      <Select
                        id="sectionCheck"
...
export default AddMasterToolNumberSpecSleeve;
