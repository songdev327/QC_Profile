...
function AddMasterToolNumberSpecTN() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/masterNumberSleeve/masterNumberSpecToolInsertTN",
          payload
        );
        if (response.data.message === "success") {
          Swal.fire({
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(
        config.api_path +
        "/masterNumberSleeve/masterNumberSpecNumberToolListTN",
        config.headers()
      );
      ...
      if (res.isConfirmed) {
        try {
          await axios
            .put(
              config.api_path +
              "/masterNumberSleeve/masterNumberSpecNumberToolupdateTN/" +
              item.id,
              {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(
            config.api_path +
            "/masterNumberSleeve/masterNumberSpecNumberTooldeleteTN/" +
            item.id,
            config.headers()
          );
          Swal.fire({
  ...
  const getToolNumberSearch = async () => {
    try {
      const response = await axios.get(
        config.api_path + "/masterSpecToolSleeve/masterSpecToolSearchSleeveTN"
      );
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${config.api_path}/upload-master-tool-spec-tn`,
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">
                    ADD NEW MASTER TOOL NUMBER SPEC ( TN MACHINE )
                  </b>
                </h3>
              </div>
                  ...
                  <div className="col-12">
                    <div className="input-group">
                      <input
                        ...
                        className="col-4 form-control mt-3 ml-2"
                        placeholder="Part no........."
                      />
                      <input
                        type="text"
...
export default AddMasterToolNumberSpecTN;
