...
function AddMasterToolNumberSpecTTC() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/masterNumber/masterNumberSpecToolInsertTTC",
          payload
        );
        if (response.data.message === "success") {
          Swal.fire({
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(
        config.api_path + "/masterNumber/masterNumberSpecNumberToolListTTC",
        config.headers()
      );
  ...
  const getToolNumberSearch = async () => {
    try {
      const response = await axios.get(
        config.api_path + "/masterSpecTool/masterSpecToolSearchTTC"
      );
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">
                    ADD NEW MASTER TOOL NUMBER SPEC ( TTC MACHINE )
                  </b>
                </h3>
              </div>
...
export default AddMasterToolNumberSpecTTC;
