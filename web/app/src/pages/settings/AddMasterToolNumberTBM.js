...
function AddMasterToolNumberTBM() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/masterToolNumber/masterToolNumberInsertTBM",
          payload
        );
        if (response.data.message === "success") {
          Swal.fire({
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(
        config.api_path + "/masterToolNumber/masterToolNumberListTBM",
        config.headers()
      );
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios
            .put(
              config.api_path + "/masterToolNumber/updateTBM/" + item.id,
              {
  ...
  const handleDelete = async (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(
            config.api_path + "/machinesToolNumber/deleteTBM/" + item.id,
            config.headers()
          );
          Swal.fire({
            title: "ลบข้อมูล",
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">ADD NEW MASTER TOOL NUMBER ( TBM MACHINE )</b>
                </h3>
              </div>
...
export default AddMasterToolNumberTBM;