...
function AddmachineShaft() {
  ...
  const fetchDataMachineList = async () => {
    try {
      const res = await axios.get(config.api_path + "/machines/listShaft", config.headers());
  ...
  const handleMachine = async (e) => {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(config.api_path + "/machine/machineRegisterShaft", payload);
  ...
  const handleUpdate = async (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.put(config.api_path + '/machines/updateShaft/' + item.id, {
  ...
  const handleDelete = async (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(config.api_path + '/machines/deleteShaft/' + item.id, config.headers());
          Swal.fire({
            title: 'ลบข้อมูล',
            text: 'ลบข้อมูลแล้ว',
            icon: 'success',
  ...
  const getMachinesSearch = async () => {
    try {
      const response = await axios.get(config.api_path + "/machines/listSearchShaft");
          ...
          <div className="signup_form w-50">
            <div className="card card-outline card-success">
              <div className="card-header text-center">
                <h3>
                  <b className="fw-bold">ADD NEW MACHINE NUMBER SHAFT</b>
                </h3>
              </div>
...
export default AddmachineShaft;
