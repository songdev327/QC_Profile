...
function AddmachineType() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/machine/machineRegisterMachineType",
          payload
        );
  ...
  const fetchDataMachineList = async () => {
    try {
      await axios
        .get(config.api_path + "/machines/listMachineType", config.headers())
        .then((res) => {
          if (res.data.message === "success") {
            setMachine_types(res.data.results);
          }
        });
    } catch (e) {
      Swal.fire({
  ...
  const handleUpdate = async (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.put(config.api_path + '/machines/updateMachineType/' + item.id, {
            machine_type: machine_type, // ใช้ค่า state ของ machineNumber
          }, config.headers())
            .then(res => {
              if (res.data.message === 'success') {
                Swal.fire({
  ...
  const handleDelete = (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(config.api_path + '/machines/deleteMachineType/' + item.id, config.headers()).then(res => {
            if (res.data.message === 'success') {
              Swal.fire({
                title: 'ลบข้อมูล',
                text: 'ลบข้อมูลแล้ว',
  ...
  const handleEditClick = (item) => {
    ...
    setMachine_type(item.machine_type); // ตั้งค่า machineNumber ใน state
  };
            ...
            <div className="card-header text-center">
              <div className="card">
                <div className="card-header">
                  <h3>
                    <b className="fw-bold">ADD NEW MACHINE TYPE</b>
                  </h3>
                </div>
                    ...
                    <div className="col-6">
                      <div className="input-group">
                        <input
                          ...
                          placeholder="Input New Machine Number....."
                        />
                      </div>
                    </div>
