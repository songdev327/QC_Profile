...
function AddpartnameShaft() {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await axios.post(
          config.api_path + "/partname/partnameRegisterShaft",
          payload
        );
  ...
  const fetchDataPartNameList = async () => {
    try {
      const res = await axios.get(config.api_path + "/partname/listShaft", config.headers());
  ...
  const handleDelete = (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.delete(config.api_path + '/partname/deleteShaft/' + item.id, config.headers()).then(res => {
            if (res.data.message === 'success') {
              Swal.fire({
                title: 'ลบข้อมูล',
                text: 'ลบข้อมูลแล้ว',
  ...
  const handleUpdate = async (item) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        try {
          await axios.put(config.api_path + '/partname/updateShaft/' + item.id, {
            Partname_Model: partname,
            Part_No: partNo,
          }, config.headers())
            .then(res => {
              ...
              <div className="card-header text-center" id="">
                <div className="card">
                  <div className="card-header">
                    <h3 className="">
                      <b className="fw-bold">ADD NEW PART MODEL NAME SHAFT (Search master PDF file)</b>
                    </h3>
                  </div>
...
export default AddpartnameShaft;
