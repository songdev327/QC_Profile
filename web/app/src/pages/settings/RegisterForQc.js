...
function RegisterForQc() {
  ...
  const fetchDataUserList = async () => {
    try {
      await axios
        .get(config.api_path + "/user/listQCEquipment", config.headers())
        .then((res) => {
  ...
  const fetchDataUserListSearch = async () => {
    try {
      const res = await axios.get(config.api_path + "/user/listQCEquipment", config.headers());
      if (res.data.message === "success") {
  ...
  return (
    <>
      <Template>
            ...
            <div className="card card-outline card-secondary mt-3">
              <div className="card-header">
                <div className="container mt-3">
                  <div className="h3 mt-3 mb-3" id="qc-equipment-system-2">
                    REGISTER NEW MEMBER TO APPLICATION
                  </div>
                  <div className="row">
                    {packages.map((item) => {
                      if (userProcess === "QC Equipment" && item.name === "QC Equipment") {
        ...
        {showForm && (
          <div className="signup_container d-flex justify-content-center">
            <div className="signup_form w-40">
              <div className="card card-outline card-secondary p-5">
                <form onSubmit={handleRegister}>
                  <div>
                    <label className="mb-2">STATUS</label>
                    <div className="status-qcEqm">
        ...
        {/* ตารางข้อมูลผู้ใช้ */}
        <div className="content-wrapper">
          <table className="mt-3 table table-bordered table-striped" id="table-qc-search">
            ...
            {filteredUserNumbers.length > 0 ? (
                filteredUserNumbers
                  .filter(item => item.process === "QC Equipment") // กรองเฉพาะ process ที่เป็น "Production"
                  .map((item) => (
                  <tr key={item.id}>
